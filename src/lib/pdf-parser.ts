...
function parseDeveloperTemplate(lines: string[]): CVFormValues {
  ...
  if (workExpIndex >= 0) {
    const endIndex = findNextSection([educationIndex, techStackIndex, languagesIndex, interestsIndex], workExpIndex, lines.length);
  ...
  // Parse education
  if (educationIndex >= 0) {
    const endIndex = findNextSection([techStackIndex, languagesIndex, interestsIndex], educationIndex, lines.length);
  ...
  if (techStackIndex >= 0) {
    const endIndex = findNextSection([languagesIndex, interestsIndex], techStackIndex, lines.length);
...
function parseDefaultTemplate(lines: string[]): CVFormValues {
  ...
  const competenciesIndex = lines.findIndex(l => l === 'Core Competencies' || l === 'CORE COMPETENCIES');
  ...
  // Parse work experience
  if (profExpIndex >= 0) {
    const endIndex = findNextSection([educationIndex, competenciesIndex, languagesIndex, interestsIndex], profExpIndex, lines.length);
  ...
  // Parse education
  if (educationIndex >= 0) {
    const endIndex = findNextSection([competenciesIndex, languagesIndex, interestsIndex], educationIndex, lines.length);
    const eduLines = lines.slice(educationIndex + 1, endIndex);
    result.education = parseEducation(eduLines);
  }
  ...
  if (competenciesIndex >= 0) {
    const endIndex = findNextSection([languagesIndex, interestsIndex], competenciesIndex, lines.length);
...
function parseVeterinaryTemplate(lines: string[]): CVFormValues {
  ...
  // Parse education
  if (educationIndex >= 0) {
    const endIndex = findNextSection([skillsIndex, languagesIndex, interestsIndex], educationIndex, lines.length);
    const eduLines = lines.slice(educationIndex + 1, endIndex);
    result.education = parseEducation(eduLines);
  }
  ...
  // Parse skills
  if (skillsIndex >= 0) {
    const endIndex = findNextSection([languagesIndex, interestsIndex], skillsIndex, lines.length);
...
function parseHeaderInfo(lines: string[]): CVFormValues['personalInfo'] {
  ...
  for (const line of lines) {
    ...
    if (line.toLowerCase().includes('github') || /^[A-Za-z0-9-]+$/.test(line)) {
      ...
      } else if (lines.indexOf(line) > 1 && /^[A-Za-z0-9_-]+$/.test(line) && line.length < 30) {
    ...
    const locationMatch = line.match(/([A-Z][a-z]+(?:\s[A-Z][a-z]+)?),\s*([A-Z][a-z]+(?:\s[A-Z][a-z]+)?)/);
...
function parseExperiences(lines: string[]): CVFormValues['experiences'] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const dateMatch = line.match(/(\w+\s+\d{4})\s*[-–]\s*(\w+\s+\d{4}|Present)\s*(?:\|\s*(.+))?/);
...
function isNewEntry(line: string): boolean {
  return line.includes('|') ||
         line.match(/^\w+\s+\d{4}/) !== null ||
         line.match(/^[A-Z]/) !== null;
}

function isCommonWord(word: string): boolean {
  const common = ['the', 'and', 'or', 'a', 'an', 'in', 'on', 'at', 'to', 'for', 'of', 'with', '|', '-', '–'];
