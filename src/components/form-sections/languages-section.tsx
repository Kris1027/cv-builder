      ...
      <CardHeader className="bg-gradient-to-r from-cyan-50 to-teal-50 rounded-t-lg">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1">Add languages you speak</CardDescription>
          </div>
          <Button 
            type='button' 
            onClick={addLanguage}
          ...
          {(field) => (
            <div className='space-y-4'>
              ...
              {field.state.value.map((_: LanguageProps, index: number) => (
                <div key={index} className='bg-gradient-to-r from-cyan-50/50 to-white p-6 rounded-xl border border-gray-200 hover:shadow-md transition-shadow'>
                  <div className='flex justify-between items-center mb-4'>
                    <h4 className='font-semibold text-lg text-gray-700'>Language {index + 1}</h4>
                    <Button
                      type='button'
                      variant='ghost'
                      size='sm'
